...
namespace {
std::uint64_t
performOperation(const aoc25::day6::Problem& problem)
{
  std::uint64_t result =
    (problem.operation == aoc25::day6::Operation::Add) ? 0 : 1;
...
std::uint64_t
performOperation2(const aoc25::day6::Problem& problem)
{
  ...
  for (auto& strOperand : strOperands) {
    while (strOperand.size() < maxDigits) {
      if (problem.alignment == aoc25::day6::Alignment::Left) {
        ...
        strOperand.insert(strOperand.begin(), 'x');
      }
    }
  }
